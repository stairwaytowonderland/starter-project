...
# CODESERVER_VERSION="${CODESERVER_VERSION:-4.112.0}"
...
TIME_MSG_LABEL= TIME_MSG_PREFIX= $BUILD_CONTEXT/$bin_dir/build.sh $REPO_NAME:devtools $REMOTE_USER --build-arg DEV_PARENT_IMAGE=brewuser --build-arg PYTHON_VERSION=latest --build-arg UNIMATRIX_ENABLED=true $* $BUILD_CONTEXT
...
TIME_MSG_LABEL= TIME_MSG_PREFIX= $BUILD_CONTEXT/$bin_dir/build.sh $REPO_NAME:codeserver $REMOTE_USER --build-arg BIND_ADDR=$CODESERVER_BIND_ADDR --build-arg DEFAULT_PASS_CHARSET='a-zA-Z0-9' --build-arg CODESERVER_VERSION=4.112.0 $* $BUILD_CONTEXT
TIME_MSG_LABEL= TIME_MSG_PREFIX= $BUILD_CONTEXT/$bin_dir/build.sh $REPO_NAME:codeserver-minimal $REMOTE_USER --build-arg BIND_ADDR=$CODESERVER_BIND_ADDR --build-arg DEFAULT_PASS_CHARSET='a-zA-Z0-9' --build-arg CODESERVER_VERSION=4.112.0 $* $BUILD_CONTEXT
